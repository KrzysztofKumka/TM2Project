...
uint8_t ADC_Init(void) {
	...
	ADC0->SC3  = ADC_SC3_AVGE_MASK | ADC_SC3_AVGS(AVGS_32);		// Włącz uśrednianie na 32 próbki, domyślnie wyzwalanie na żądanie
	...
	if(ADC0->SC3 & ADC_SC3_CALF_MASK)
{
	...
	ADC0->SC3 &= ~(ADC_SC3_AVGE_MASK | ADC_SC3_AVGS(AVGS_32));  // Wyłącz uśrednianie sprzętowe
	ADC0->SC1[0] = ADC_SC1_ADCH(4);						// Zablokuj przetwornik ADC0
	ADC0->CFG2 |= ADC_CFG2_ADHSC_MASK;					// Włącz tryb szybkiej konwersji
	ADC0->CFG1 = ADC_CFG1_ADICLK(ADICLK_BUS_2) | ADC_CFG1_ADIV(ADIV_1) | (~ADC_CFG1_ADLSMP_MASK) | ADC_CFG1_MODE(MODE_8);	// Zegar ADCK równy 10.49MHz, rozdzielczość 8 bitów, krótki czas prókowania
...
uint8_t setVolume(void) {
	if((ADC0->SC1[0] & ADC_SC1_COCO_MASK) != 0) {
